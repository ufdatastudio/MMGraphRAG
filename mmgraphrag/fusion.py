...
def get_nearby_chunks(data, index):
    ...
    end_index = min(
        len(data) - 1, index + 1
    )  # If it's the last number, only take itself and the previous one
...
def align_single_image_entity(img_entity_name, text_chunks):
    ...
    alignment_prompt_user = PROMPTS["image_entity_alignment_user"].format(
        entity_type=entity_type,
        img_entity=img_entity_name,
        img_entity_description=img_entity_description,
        chunk_text=nearby_chunks,
    )
    aligned_image_entity = get_mmllm_response(
        alignment_prompt_user, PROMPTS["image_entity_alignment_system"], img_base
    )
...
def judge_image_entity_alignment(
    ...
    image_entity_judgement_user = PROMPTS["image_entity_judgement_user"].format(
        img_entity=image_entity_name,
        img_entity_description=image_entity_description,
        possible_matched_entities=possible_image_matched_entities,
        chunk_text=nearby_chunks,
    )
    matched_entity_name = get_llm_response(
        image_entity_judgement_user, PROMPTS["image_entity_judgement_system"]
    )
...
def get_possible_entities_image_clustering(
    ...
    nearby_relationship_list = sorted(
        nearby_relationship_list, key=lambda x: x["weight"], reverse=True
    )
    ...
    for relation in nearby_relationship_list:
        ...
        similarity_matrix[
            tgt_idx, src_idx
        ] *= weight  # Ensure adjacency matrix is symmetric
    ...
    dbscan = DBSCAN(
        eps=0.5, min_samples=min_samples
    ...
    labels = [
        labels[entity_names.index(entity["entity_name"])]
...
def get_possible_entities_text_clustering(
    ...
    # Output labels in the order of nearby_text_entity_list
    labels = [
        entity_labels[entity["entity_name"]] for entity in nearby_text_entity_list
    ]

    # Step 3: Classify image entities into clustering categories
    image_entity_with_labels = []
    input_embeddings = encode(
        [entity["description"] for entity in filtered_image_entity_list]
    )
    nn = NearestNeighbors(n_neighbors=1, metric="cosine").fit(embeddings)
    for image_entity, input_embedding in zip(
        filtered_image_entity_list, input_embeddings
    ):
        _, nearest_idx = nn.kneighbors([input_embedding])
        target_label = labels[nearest_idx[0][0]]
...
"""
    ...
    merged_entities = get_llm_response(
        cur_prompt=prompt_user, system_content=prompt_system
    )
    normalized_merged_entities = normalize_to_json_list(merged_entities)
    return [
        item
        for item in normalized_merged_entities
...
def extract_image_entities(img_entity_name):
    ...
    image_knowledge_graph_path = os.path.join(
        working_dir,
    ...
    if not os.path.exists(image_knowledge_graph_path):
        logger.warning(f"GraphML file not found: {image_knowledge_graph_path}")
        return
...
def enhance_image_entities(enhanced_image_entity_list, nearby_chunks):
    enhance_image_entity_user = PROMPTS["enhance_image_entity_user"].format(
        enhanced_image_entity_list=enhanced_image_entity_list, chunk_text=nearby_chunks
    )
    enhanced_image_entities = get_llm_response(
        enhance_image_entity_user, PROMPTS["enhance_image_entity_system"]
    )
...
def image_knowledge_graph_alignment(image_entity_name):
    ...
    filtered_image_entity_list = [
        entity
        for entity in image_entity_list
        if entity["entity_type"] not in exclude_types
    ]
    nearby_text_entity_list = get_nearby_entities(
        chunk_knowledge_graph, chunk_order_index
    )
    nearby_relationship_list = get_nearby_relationships(
        chunk_knowledge_graph, chunk_order_index
    )
    image_entity_with_labels, text_clustering_results = (
        get_possible_entities_text_clustering(
            filtered_image_entity_list,
            nearby_text_entity_list,
            nearby_relationship_list,
        )
    )
    aligned_text_entity_list = judge_text_entity_alignment_clustering(
        image_entity_with_labels, text_clustering_results
    )
...
def enhanced_image_knowledge_graph(aligned_text_entity_list, image_entity_name):
    ...
    enhanced_img_kg_path = os.path.join(
        working_dir,
    ...
    for entity in aligned_text_entity_list:
        if "source_image_entities" in entity and entity["source_image_entities"]:
            ...
            logger.warning(
                f"Warning: 'source_image_entities' is missing or empty for entity {entity}. Skipping."
            )
    # Filter out image entities that have a matching 'entity_name'
    enhanced_image_entity_list = [
        entity
        for entity in filtered_image_entity_list
        if entity["entity_name"] not in source_image_entities
    ]
    enhanced_image_entities = enhance_image_entities(
        enhanced_image_entity_list, nearby_chunks
    )
    ...
    for entity in enhanced_image_entities:
        original_name = entity.get(
            "original_name", None
        )  # Use .get() to avoid KeyError
        if original_name is None:
            logger.warning(
                f"Warning: 'original_name' is missing for entity {entity}. Skipping update."
            )
        ...
        if "description" not in entity:
            logger.warning(
                f"Warning: 'description' is missing for entity {entity}. Skipping update."
            )
            continue
        ...
        for node in G.nodes(data=True):
            ...
            if original_name == node_id:
                ...
                G.nodes[entity["entity_name"]].update(
                    {"description": entity["description"]}
                )
...
def image_knowledge_graph_update(enhanced_img_kg_path, image_entity_name):
    ...
    possible_image_matched_entities = get_possible_entities_image_clustering(
        image_entity_description, nearby_text_entity_list, nearby_relationship_list
    )
    matched_entity_name = judge_image_entity_alignment(
        image_entity_name,
        image_entity_description,
        possible_image_matched_entities,
        nearby_chunks,
    ...
    matched_entity_name_normalized = (
        matched_entity_name.strip().replace(" ", "").replace("\\", "").lower()
    )
    ...
    for entity in nearby_text_entity_list:
        entity_name_normalized = (
            entity["entity_name"].strip().replace(" ", "").replace("\\", "").lower()
        )
        ...
        if matched_entity_name_normalized == entity_name_normalized:
            ...
            if source_node_id is None:
                raise ValueError(
                    "No node with entity_type 'ORI_IMG' found in the graph."
                )
...
def merge_graphs(
    image_graph_path, graph_path, aligned_text_entity_list, image_entity_name
):
    global_config_path = os.path.join(cache_path, "global_config.csv")
    global_config = read_config_to_dict(global_config_path)
    working_dir = global_config["working_dir"]
    merged_kg_path = os.path.join(
        working_dir, f"graph_merged_{image_entity_name}.graphml"
    )
    ...
    # If graph loading fails, issue error and return
    if image_graph is None or text_graph is None:
        logger.error(f"Failed to load graphs, please check file paths.")
        return
    ...
    for entity_info in aligned_text_entity_list:
        ...
        if source_image_entity in image_graph.nodes:
            source_id_image = image_graph.nodes[source_image_entity].get(
                "source_id", ""
            )
        else:
            logger.warning(f"Node {source_image_entity} does not exist in image graph")
            continue

        if source_text_entity in text_graph.nodes:
            source_id_text = text_graph.nodes[source_text_entity].get("source_id", "")
        else:
            logger.warning(f"Node {source_text_entity} does not exist in text graph")
            continue
                    ...
                    if target_edge_data:
                        ...
                        for key in edge_data:
                            if key in ["weight", "description", "source_id", "order"]:
                                ...
                                target_edge_data[key] = edge_data.get(
                                    key, target_edge_data.get(key)
                                )
                    else:
        ...
        merged_graph = nx.relabel_nodes(
            merged_graph, {target_entity: ensure_quoted(entity_name)}
        )
...
async def fusion(img_ids):
    ...
    for image_entity_name in img_ids:
        ...
        enhanced_img_kg_path = enhanced_image_knowledge_graph(
            aligned_text_entity_list, image_entity_name
        )
        image_graph_path = image_knowledge_graph_update(
            enhanced_img_kg_path, image_entity_name
        )
        graph_path = merge_graphs(
            image_graph_path, graph_path, aligned_text_entity_list, image_entity_name
        )
